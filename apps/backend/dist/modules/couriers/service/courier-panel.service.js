...
exports.courierPanelService = exports.CourierPanelService = void 0;
const prisma_1 = require("../../../infra/database/prisma");
const AppError_1 = require("../../../shared/errors/AppError");
class CourierPanelService {
    async listCouriersForAssignment(filters) {
        const where = {
            deletedAt: null,
            status: 'ACTIVE',
        ...
        return Promise.all(couriers.map(async (c) => {
            const activeDeliveries = await prisma_1.prisma.assignment.count({
                where: {
                    courierId: c.id,
                    deletedAt: null,
            ...
            const [todayUrbanRoute, activeIntermunicipalRoutes] = await Promise.all([
                prisma_1.prisma.courierRoute.findUnique({
    ...
    async getCourierDetail(courierId) {
        ...
        const [activeAssignments, completedAssignments, incidents, gpsLogs] = await Promise.all([
            ...
            prisma_1.prisma.gpsLog.findMany({
    ...
    async updateConnection(courierUserId) {
...
exports.CourierPanelService = CourierPanelService;
exports.courierPanelService = new CourierPanelService();
