...
router.get('/:id/status-history', (0, role_middleware_1.requirePermission)('deliveries.read', 'calls.read', 'audit.read'), async (req, res, next) => {
    try {
        const data = await delivery_status_service_1.deliveryStatusService.getHistory(req.params.id);
...
router.post('/:id/action', (0, role_middleware_1.requirePermission)('calls.write', 'deliveries.write'), (0, validate_middleware_1.validate)(actionSchema), async (req, res, next) => {
    try {
        ...
        switch (action) {
            case 'CONFIRM':
                result = await delivery_status_service_1.deliveryStatusService.confirmForDelivery(req.params.id, req.user.sub, observations);
                break;
            case 'PENDING':
                if (!pendingSubreason) {
                ...
                result = await delivery_status_service_1.deliveryStatusService.setPending(req.params.id, req.user.sub, pendingSubreason, observations);
                break;
            case 'DEACTIVATE':
                ...
                result = await delivery_status_service_1.deliveryStatusService.deactivate(req.params.id, req.user.sub, deactivationReason, observations);
                break;
            case 'REACTIVATE':
                result = await delivery_status_service_1.deliveryStatusService.reactivate(req.params.id, req.user.sub, observations);
                break;
            case 'RESCHEDULE':
                result = await delivery_status_service_1.deliveryStatusService.transition(req.params.id, req.user.sub, {
